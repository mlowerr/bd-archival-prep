...
# Apply an existing file-packing report:
/path/to/repo/scripts/unix/apply-blu-ray-file-recommendations.sh \
...
/path/to/repo/scripts/unix/lib/plan_and_move_folders.sh [--disk-size PLAN] [--base-name NAME] [--include-disk-number | --disk-number-with-total] [--include-can-add]
...
Pass `--include-can-add` to either plan-and-move driver to append each disk's
unused capacity as the final portion of its folder name. For example, a disk using
...
### 5) Apply Blu-ray file recommendations (Unix only)

**Script**
- `scripts/unix/apply-blu-ray-file-recommendations.sh`

**Input**
- an existing `.archival-prep/blu-ray-file-recommendations.txt` generated by `file-size-recommendations.sh`

**Behavior**
- Prompts the user to choose one of the three plan sections exactly as written in the report.
- Prompts for the disk base name.
- Prompts for the recommendation file path and destination root if those were not supplied on the command line.
- Parses the selected plan strictly and errors out if that selected section is malformed or contains no executable disk assignments.
- Uses the report's `Size used: ... GiB` value in each disk folder name:
  - `[Base Name]-Disk[#]-[USED DISK CAPACITY]GiB`
- Requires an explicit `YES` confirmation before moving any files.
